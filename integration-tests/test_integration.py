    ...
    def test_invalid_evidence_id(self):
        ...
        try:
            response = self.session.get(
                f"{API_BASE_URL}/api/v1/evidence/invalid_id/results"
            )
...
class TestUploadFunctionality:
    ...
    def test_evidence_upload_basic(self):
        ...
        try:
            with open(self.temp_file.name, "rb") as f:
                ...
                data = {"device_id": "test_camera_001", "evidence_type": "test_video"}

                response = self.session.post(
                    f"{API_BASE_URL}/api/v1/evidence/upload", files=files, data=data
                )
    ...
    def test_upload_without_file(self):
        ...
        try:
            data = {"device_id": "test_camera_002", "evidence_type": "test_video"}

            response = self.session.post(
                f"{API_BASE_URL}/api/v1/evidence/upload", data=data
            )
...
if __name__ == "__main__":
    ...
    pytest.main(
        [__file__, "-v", "--tb=short", "-k", "not (test_upload or integration)"]
    )
