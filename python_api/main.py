...
async def process_evidence_background(
    ...
    try:
        ...
        try:
            results = await evidence_processor.process_video_evidence(
                file_path, device_id
            )
    ...
    if evidence_id not in completed_evidence:
        if evidence_id in processing_status:
            ...
            if status.status == "processing":
                ...
                raise HTTPException(
                    status_code=500, detail=f"Processing failed: {status.error_message}"
                )

        raise HTTPException(
            status_code=404, detail="Evidence not found or processing not completed"
        )
...
async def verify_evidence(
    evidence_id: str, request: VerificationRequest, auth: bool = Depends(verify_token)
):
    ...
    try:
        ...
        if request.include_ai_analysis:
            ...
            verification_result["ai_verification"] = {
                ...
                "faces_detected": ai_results.get("faces_summary", {}).get(
                    "total_faces", 0
                ),
                "quality_score": ai_results.get("quality_stats", {}).get(
                    "mean_quality", 0
                ),
...
async def generate_court_report(
    evidence_id: str, request: CourtReportRequest, auth: bool = Depends(verify_token)
):
    ...
    try:
        ...
        court_report = {
            ...
            # Chain of custody
            "chain_of_custody": {
                "captured_at": evidence["processing_results"].get(
...
async def list_evidence(
    ...
    try:
        ...
        return {"total": total, "skip": skip, "limit": limit, "evidence": paginated}
