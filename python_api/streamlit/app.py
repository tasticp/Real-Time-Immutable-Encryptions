    ...
    fig = go.Figure(data=[
        go.Table(
            header=dict(values=list(df.columns),
                      ...
                      align='left'),
            cells=dict(values=[df.Network, df.Status, df.Confirmations, df["Last Anchor"]],
                      fill_color='lavender',
                      align='left'))
    ])
...
def main():
    # Header
    st.markdown('<h1 class="main-header">⚖️ Court Evidence Verification Portal</h1>', 
                unsafe_allow_html=True)
    ...
    page = st.sidebar.selectbox(
        "Select Page",
        ["Dashboard", "Upload Evidence", "Evidence Details", "Court Report", "Blockchain Verification", "Legal Compliance"]
    )
    ...
    for evidence in recent_evidence:
        with st.container():
            ...
            with col3:
                ...
                st.markdown(f'<span class="{status_class}">● {evidence["status"].title()}</span>', 
                           unsafe_allow_html=True)
    ...
    with st.form("upload_form"):
        ...
        with col2:
            evidence_type = st.selectbox(
                "Evidence Type",
                ["Aerial Surveillance", "Security Camera", "Body Camera", 
                 "Traffic Camera", "Dash Camera", "Other"],
        ...
        if submitted and uploaded_file:
            with st.spinner("Uploading and starting analysis..."):
                ...
                **Evidence uploaded successfully!**
                ...
                st.session_state.evidence_id = f"ev_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
...
def evidence_details_page():
    ...
    if hasattr(st.session_state, 'selected_evidence'):
        ...
        frames_df = pd.DataFrame({
            ...
            'Motion': [True, False, True, True, False, True, True, False, True, False]
        })
        ...
        face_results = [
            {"face_id": "F_001", "confidence": 0.98, "frames": 15, "time_on_screen": "0:45"},
            {"face_id": "F_002", "confidence": 0.95, "frames": 8, "time_on_screen": "0:24"},
            {"face_id": "F_003", "confidence": 0.92, "frames": 12, "time_on_screen": "0:36"}
        ]
        
        for face in face_results:
            with st.expander(f"Face {face['face_id']} - {face['confidence']:.0%} confidence"):
                col1, col2, col3, col4 = st.columns(4)
...
def court_report_page():
    ...
    report_type = st.selectbox(
        "Report Type",
        ["Full Court Report", "Summary", "Expert Testimony", "Technical Specifications"]
    )
    
    jurisdiction = st.selectbox(
        "Jurisdiction",
        ["US Federal", "California", "New York", "Texas", "UK", "EU"]
    )
        ...
        with st.spinner("Generating comprehensive court report..."):
            ...
            **Generated**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  
            ...
            with col2:
                st.download_button(
                    "📊 Download JSON",
                    data=json.dumps({"evidence_id": evidence_id, "report": "mock_data"}, indent=2),
...
def blockchain_page():
    ...
    timeline_data = [
        ...
        {"time": "10:30:05", "event": "AES-256 encryption applied", "status": "success"},
        {"time": "10:30:10", "event": "Bitcoin transaction sent", "status": "success"},
        {"time": "10:30:15", "event": "Ethereum contract called", "status": "success"},
        {"time": "10:30:20", "event": "Private chain anchor created", "status": "success"},
        {"time": "10:35:30", "event": "Bitcoin confirmation received", "status": "success"},
        {"time": "10:37:45", "event": "Ethereum confirmation received", "status": "success"}
    ]
